...
import static dk.statsbiblioteket.digital_pligtaflevering_aviser.doms.ToolResultsReport.OK_COUNT_FAIL_LIST_RENDERER;
...
//            });
    ...
    protected Consumer<Exception> noLogging = throwable -> {
    };
...
//    protected Function<IdValue<DomsItem, Either<Exception, ToolResult>>, String> toString = c -> c.id() + " " + c.value().isSuccess() + ": " + c.value().getHumanlyReadableMessage();
...
//    protected Function<List<Either<ToolThrewException, ToolResult>>, String> f = eitherList -> toString.apply(tf.apply(id0, eitherList));
    ...
    @Test
    public void noItems() {
        List<IdValue<DomsItem, ToolResult>> l = new ArrayList<DomsItem>().stream()
                .map(IdValue::create)
                .map(c -> c.map(v -> Either.right(ToolResult.ok(String.valueOf(v)))))
                .map(c -> c.map(Either::get))
                .collect(toList());

        String report = OK_COUNT_FAIL_LIST_RENDERER.apply(l, emptyList());

        Assert.assertEquals("0 ok", report);
    }
