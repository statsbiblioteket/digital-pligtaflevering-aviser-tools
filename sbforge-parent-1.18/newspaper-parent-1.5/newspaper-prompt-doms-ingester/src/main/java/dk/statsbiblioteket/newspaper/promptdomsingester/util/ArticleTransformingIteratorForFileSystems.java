...
import dk.statsbiblioteket.medieplatform.autonomous.iterator.filesystem.transforming.CommonTransformingIterator;
import dk.statsbiblioteket.medieplatform.autonomous.iterator.filesystem.transforming.VirtualIteratorForFileSystems;
...
public class ArticleTransformingIteratorForFileSystems extends CommonTransformingIterator {
    ...
    public static final String DATA_FILE_PATTERN_VALUE = ".*\\.pdf$";
     ...
     */
    public ArticleTransformingIteratorForFileSystems(File id, String groupingPattern, String dataFilePattern,
     ...
     * @param ignoredFiles    Files to ignore during transformation.
     */
    protected ArticleTransformingIteratorForFileSystems(File id, File prefix, String groupingPattern, String dataFilePattern,
                                                        String checksumPostfix, List<String> ignoredFiles) {
    ...
    protected Iterator<DelegatingTreeIterator> initializeChildrenIterator() {
        ...
        for (File child : children) {
            result.add(
                    new ArticleTransformingIteratorForFileSystems(
    ...
    protected boolean containsDatafiles(Collection<File> files) {
        for (File attribute : files) {
            if (attribute.getName().contains("article")) {
                return true;
            }
        }
        return super.containsDatafiles(files);
    }
}
